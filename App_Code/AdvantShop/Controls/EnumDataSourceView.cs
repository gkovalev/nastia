...
namespace AdvantShop.Controls
{
    public class EnumDataSourceView : DataSourceView
    {
        ...
        private readonly int? _exceptValue;
        ...
        {
            return GetEnumItems(_enumType, _exceptValue, _exceptValues);
        }
        private static IList GetEnumItems(Type type, int? exceptValue, List<int> exceptValues )
        {
            if (type.BaseType != typeof(Enum))
                throw new ArgumentException(@"Type must be an enumeration.", "type");
            var vals = Enum.GetValues(type).Cast<object>();
            //if (exceptValue != null)
            //    vals = vals.Where(val => (int)val != exceptValue.Value);
