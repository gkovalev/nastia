...
namespace AdvantShop.Controls
{
    ...
    public class AdvGridView : GridView
    {
        ...
        //public eToolTipTarget ToolTipTarget
        //{
        //    get { return _toolTipTarget; }
        //    set { _toolTipTarget = value; }
        //}
        ...
        protected override void OnRowCreated(GridViewRowEventArgs e)
        {
                ...
                if (!string.IsNullOrEmpty(DataFieldForEditURLParam))
                {
                    ...
                    if (((DataRowView)e.Row.DataItem).Row.Table.Columns.Contains("ID"))
                    {
                        productId = (((DataRowView)e.Row.DataItem)["ID"] is DBNull)
                                        ? -1
                                        : Convert.ToInt32(((DataRowView)e.Row.DataItem)["ID"]);
                    }
                }
            }
            ...
            if (e.Row.RowType != DataControlRowType.Header)
            {
                ...
                {
                    if (i == TooltipTextCellIndex && ProductService.GetCountOfCategoriesByProductId(Convert.ToInt32(productId)) > 0)
                    {
                        if (((DataRowView)e.Row.DataItem).Row.Table.Columns.Contains("typeItem") && Convert.ToInt32(((DataRowView)e.Row.DataItem)["typeItem"]) == 1)
                        {
                            var tip = ProductService.CreateTooltipContent(Convert.ToInt32(productId));
                            if (tip.Length > 0)
                            {
                                e.Row.Cells[i].Attributes.Add("class", "txttooltip");
                                e.Row.Cells[i].Attributes.Add("abbr", tip);
                            }
                        }
                    }
                    ...
                    if (i > 0 && i < e.Row.Cells.Count - 1)
                    {
                        ...
                        {
                            //e.Row.Cells(i).Attributes.Add("onclick", Page.ClientScript.GetPostBackEventReference(Me, "Edit$" & e.Row.RowIndex.ToString()))
                        }
                        else
                        {
