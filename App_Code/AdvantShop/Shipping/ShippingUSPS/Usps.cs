...
namespace AdvantShop.Shipping
{
    public class Usps : IShippingMethod
    {
        ...
        public Usps(Dictionary<string, string> parameters)
        {
            try
            {
                Rate = parameters.ElementOrDefault(UspsTemplate.Rate).TryParseDecimal();
                Extracharge = parameters.ElementOrDefault(UspsTemplate.Extracharge).TryParseDecimal();
            ...
            using (var tr = new XmlTextReader(sr))
            {
                do
                {
                    ...
                    if ((tr.Name == serviceTag) && (tr.NodeType == XmlNodeType.Element))
                    {
                        ...
                        {
                            var shippingRate = (Rate > 0) ? Convert.ToDecimal(postalRate, new CultureInfo("en-US"))* Rate + Extracharge
                                                          : Convert.ToDecimal(postalRate, new CultureInfo("en-US")) + Extracharge;
                            //if (Rate > 0)
                            //{
                            //    shippingRate *= Rate;
                            //}
