...
namespace AdvantShop.Shipping
{
    ...
    {
        private const decimal MaxPackageWeight = 68.03M;
        ...
        public Ups(Dictionary<string, string> parameters)
        {
            try
            {
                ...
                Extracharge = parameters.ElementOrDefault(UpsTemplate.Extracharge).TryParseDecimal();
                Rate = parameters.ElementOrDefault(UpsTemplate.Rate).TryParseDecimal();
        ...
        private RateRequest CreateRateRequest(RateService rate)
        {
            ...
            decimal weight = MeasureUnits.ConvertWeight(ShoppingCart.TotalShippingWeight, MeasureUnits.WeightUnit.Kilogramm, MeasureUnits.WeightUnit.Pound);
            ...
            else
            {
                ...
                if (IsPackageTooHeavy(weight))
                {
                    totalPackagesWeights = Convert.ToInt32(Math.Ceiling(weight / MaxPackageWeight));
                }
        ...
        private List<ShippingOption> ParseAnswer(RateResponse rateResponse)
        {
            ...
            foreach (var rateDetail in rateResponse.RatedShipment)
            {
                ...
                var shippingRate = (Rate > 0) ? Convert.ToDecimal(rateDetail.TotalCharges.MonetaryValue, new CultureInfo("en-US")) * Rate + Extracharge
                                              : Convert.ToDecimal(rateDetail.TotalCharges.MonetaryValue, new CultureInfo("en-US")) + Extracharge;
