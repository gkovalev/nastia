    ...
    public class FedEx : IShippingMethod
    {
        ...
        public FedEx(Dictionary<string, string> parameters)
        {
            try
            {
                ...
                Rate = parameters.ElementOrDefault(FedExTemplate.Rate).TryParseDecimal();
                Extracharge = parameters.ElementOrDefault(FedExTemplate.Extracharge).TryParseDecimal();
        ...
        private void SetShipmentDetails(RateRequest request)
        {
            ...
            request.RequestedShipment.TotalInsuredValue.Amount = TotalPrice; // Не использовать ShoppingCart.TotalPrice - не доступен из потока
        ...
        private void SetIndividualPackageLineItems(RateRequest request)
        {
            ...
            if (!IsPackageTooHeavy(weight))
            {
                ...
                request.RequestedShipment.RequestedPackageLineItems[0].InsuredValue.Amount = TotalPrice;
            ...
            else
            {
                ...
                decimal orderSubTotal2 = ShoppingCart.TotalPrice / totalPackages;
