        ...
        private void CreateAux()
        {
            using (var db = new SQLDataAccess())
            {
                ...
                using (var read = db.cmd.ExecuteReader())
                {
                    ...
                    while (read.Read())
                        _sw.WriteLine("<li><a href='{0}'>{1}</a></li>", _prefUrl + UrlService.GetLink(ParamType.StaticPage, Convert.ToString(read["UrlPath"]), Convert.ToInt32(read["StaticPageID"])), read["PageName"]);
            ...
            using (var db = new SQLDataAccess())
            {
                ...
                using (var read = db.cmd.ExecuteReader())
                {
                    ...
                    while (read.Read())
                    {
                        ...
                        {
                            _sw.WriteLine("<li><a href='{0}'>{1}</a>", _prefUrl + UrlService.GetLink(ParamType.Category, Convert.ToString(read["UrlPath"]), Convert.ToInt32(read["CategoryID"])), read["Name"]);
        ...
        private void GetSubCategories(int categoryId)
        {
            ...
            using (var db = new SQLDataAccess())
            {
                ...
                using (var read = db.cmd.ExecuteReader())
                {
                    ...
                    {
                        _sw.WriteLine("<li>");
                        _sw.WriteLine("<a href='{0}'>{1}</a>", _prefUrl + UrlService.GetLink(ParamType.Category, Convert.ToString(read["UrlPath"]), Convert.ToInt32(read["CategoryID"])), HttpUtility.HtmlEncode(read["Name"]));
                        GetSubCategories((int) read["CategoryID"]);
                        GetProducts((int) read["CategoryID"]);
                        _sw.WriteLine("</li>");
                    }
        ...
        private void GetProducts(int categoryId)
        {
            ...
            using (var db = new SQLDataAccess())
            {
                db.cmd.CommandText = "SELECT Product.[ProductID] as ProductID, Product.[Name] as Name,ProductCategories.CategoryID as ParentCategory,[UrlPath]  FROM [Catalog].[ProductCategories]" +
                                     " INNER JOIN [Catalog].[Product] ON [Product].ProductID = ProductCategories.ProductID WHERE HirecalEnabled =1 and [Enabled] = 1 and ProductCategories.Main = 1 " +
                                     "and (select Count(CategoryID) from Catalog.ProductCategories where ProductID=[Product].[ProductID])<> 0 and CategoryID=@categoryId ";
                db.cmd.Parameters.AddWithValue("@categoryId", categoryId);
                db.cnOpen();
                using (var read = db.cmd.ExecuteReader())
                    while (read.Read())
                        _sw.WriteLine("<li><a href='{0}'>{1}</a></li>", _prefUrl + UrlService.GetLink(ParamType.Product, Convert.ToString(read["UrlPath"]), Convert.ToInt32(read["ProductID"])), HttpUtility.HtmlEncode(read["Name"]));
            }
            _sw.WriteLine("</ul>");
        }
