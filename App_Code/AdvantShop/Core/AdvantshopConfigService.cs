...
namespace AdvantShop.Core
{
    ...
    public class AdvantshopConfigService
    {
        ...
        public static List<object> GetDropdownPayments()
        {
            ...
            {
                PaymentType value;
                if (payment.Attributes == null || !PaymentType.TryParse(payment.Attributes["name"].Value, out value) || !Convert.ToBoolean(payment.Attributes["enabled"].Value))
                {
                    continue;
                }
        ...
        public static List<object> GetDropdownShippings()
        {
            ...
            {
                ShippingType value;
                if (shipping.Attributes == null || !ShippingType.TryParse(shipping.Attributes["name"].Value, out value) || !Convert.ToBoolean(shipping.Attributes["enabled"].Value))
                {
                    continue;
                }
        ...
        public static Dictionary<string, bool> GetActivities(List<XmlNode> collection)
        {
            ...
            foreach (var item in collection)
            {
                ...
                {
                    result.Add(item.Attributes["name"].Value, Convert.ToBoolean(item.Attributes["enabled"].Value));
                }
            }
            return result;
        }
        ...
        public static bool GetActivity(List<XmlNode> collection, string value)
        {
            ...
            if (setting != null)
            {
                return Convert.ToBoolean(setting.Attributes["enabled"].Value);
            }
            return true;
        }
