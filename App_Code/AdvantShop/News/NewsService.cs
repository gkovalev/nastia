...
namespace AdvantShop.News
{
    public class NewsService
    {
        ...
        {
            return SQLDataHelper.GetInt(SQLDataAccess.ExecuteScalar("[Settings].[sp_GetLastNewsId]", CommandType.StoredProcedure));
        }
        ...
        public static int InsertNews(NewsItem news)
        {
            ...
            if (news.Meta != null)
            {
                if (!news.Meta.Title.IsNullOrEmpty() || !news.Meta.MetaKeywords.IsNullOrEmpty() || !news.Meta.MetaDescription.IsNullOrEmpty())
                {
                    news.Meta.ObjId = id;
