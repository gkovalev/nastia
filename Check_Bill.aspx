...
<%@ Page Language="C#" AutoEventWireup="true" ValidateRequest="false" CodeFile="Check_Bill.aspx.cs"
    Inherits="Check_Bill" %>
...
<body onload="position_this_window();window.print();">
    ...
    <div>
        <div class="Vnimanie">
            ...
            на складе. Товар отпускается по факту прихода денег на р/с Поставщика, самовывозом,
                    ...
                    <asp:Repeater runat="server" ID="rptOrderItems" DataSource="<%# Order.OrderItems %>">
                        <ItemTemplate>
                            <tr bgcolor="white">
                                ...
                                <td>
                                    <font class="sc"><b>
                                        <nobr><asp:Literal runat="server" Text='<%# Eval("Name") %>' /></nobr>
                                    </b></font>
                                    <td>
                                        <font class="sc">
                                    ...
                                    <td>
                                        <font class="sc">
                                            <asp:Literal ID="Literal4" runat="server" Text='<%# Math.Round((Convert.ToDecimal(Eval("Price"))) / Order.OrderCurrency.CurrencyValue, 2).ToString("##,##0.00") + " руб."%>' /></font>
                                    </td>
                                    <td>
                                        <font class="sc">
                                            <asp:Literal ID="Literal5" runat="server" Text='<%# Math.Round((Convert.ToDecimal(Eval("Price"))) / Order.OrderCurrency.CurrencyValue, 2).ToString("##,##0.00") + " руб." %>' /></font>
                                    </td>
                                    <td>
                                        <font class="sc">
                                    ...
                                    <td style="text-align: right">
                                        <font class="sc">
                                            <asp:Literal ID="Literal7" runat="server" Text='<%# Math.Round((Convert.ToDecimal(Eval("Price"))) * (Convert.ToInt32(Eval("Amount"))) / Order.OrderCurrency.CurrencyValue, 2).ToString("##,##0.00") + " руб." %>' /></font>
                                    </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
                    <tr bgcolor="white" id="trShipping" runat="server" visible="<%# ShippingPrice != 0 %>">
                        <td>
                            <font class="sc">
                                <asp:Literal ID="Literal1" runat="server" Text="<%# rptOrderItems.Items.Count + 1%>" />
                            </font>
                        </td>
                        <td colspan="5">
                            <font class="sc"><b>
                        ...
                        <td style="text-align: right">
                            <font class="sc">
                                <asp:Literal ID="Literal2" runat="server" Text='<%# AdvantShop.Catalog.CatalogService.GetStringPrice(ShippingPrice, 0, 1) %>' /></font>
                        </td>
                    </tr>
                </tbody>
            </table>
        ...
        <div>
            Всего наименований
            <% Response.Write(rptOrderItems.Items.Count + (ShippingPrice != 0 ? 1 : 0)); %>,
            на сумму
