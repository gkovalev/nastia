$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            constructor(data.Themes, data.Colors, data.Backgrounds, data.Structures, data.DesignCurrent, data.isTrial);
        },
        error: function (data) {
...
function constructor(arrayTheme, arrayColor, arrayBackground, arrayStructures, designDefault, showOnLoad) {
    ...
    var tpl = "<div><input type='radio' name='{0}' id='{1}' value='{2}' /><label for='{1}'>{3}</label></div>";
    $.each(arrayTheme, function (idx, el) {
        html += String.Format(tpl, "theme", "theme" + el.Name, el.Name.toLowerCase(), el.Title);
    });
    html += "</div>";
    ...
    $.each(arrayColor, function (idx, el) {
        html += String.Format(tpl, "color", "color" + el.Name, el.Name.toLowerCase(), el.Title);
    });
    html += "</div>";
    html += "<br class='clear'/></div>";
    ...
    $.each(arrayBackground, function (idx, el) {
        html += String.Format(tpl, "background", "background" + el.Name, el.Name.toLowerCase(), el.Title);
    });
    html += "</div>";
    html += "<br class='clear'/></div>";
    /****/
    ...
    $("div.constructor-themes", "div.constructor").click(function (e) {
        ...
        var path = String.Format("design/themes/{0}/css/styles.css", el.val());
    ...
    $("div.constructor-colors", "div.constructor").click(function (e) {
        ...
        var path = String.Format("design/colors/{0}/css/styles.css", el.val());
    ...
    $("div.constructor-background", "div.constructor").click(function (e) {
        ...
        var path = String.Format("design/backgrounds/{0}/css/styles.css", el.val());
        $("#themecss").attr("href", path.toLowerCase());
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data == "error") {
                ...
                var design = { Theme: theme.val(), ColorScheme: color.val(), Background: background.val(), Structure: structure.val() };
...
function designBack() {
    ...
    if (designDefault.Theme != "_none") {
        $("#themecss").attr("href", "design/themes/" + designDefault.Theme + "/css/styles.css");
    } else {
        $("#themecss").attr("href", "design/backgrounds/" + designDefault.Background + "/css/styles.css");
    }

    $("#colorcss").attr("href", "design/colors/" + designDefault.ColorScheme + "/css/styles.css");
