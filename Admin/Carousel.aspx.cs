...
public partial class Admin_Carousel : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            _paging.AddParam(new SqlParam { ParamName = "@Type", ParamValue = PhotoType.Carousel.ToString() });
    ...
    protected void lbDeleteSelected_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                ...
                {
                    CarouselService.DeleteCarousel(Convert.ToInt32(id));
                }
            }
            else
            {
        ...
        if (grid.UpdatedRow != null)
        {
            ...
            if (int.TryParse(grid.UpdatedRow["SortOrder"], out sortOrder))
            {
                var car = new Carousel()
                              {
                                  CarouselID = Convert.ToInt32(grid.UpdatedRow["ID"]),
                                  URL = grid.UpdatedRow["URL"],
                                  SortOrder = sortOrder,
                                  Enabled = Convert.ToBoolean(grid.UpdatedRow["Enabled"])
                              };
    ...
    protected void lbSetActive_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    CarouselService.SetActive(Convert.ToInt32(id), true);
                }
            }
            else
            {
    ...
    protected void lbSetNotActive_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    CarouselService.SetActive(Convert.ToInt32(id), false);
                }
            }
            else
            {
