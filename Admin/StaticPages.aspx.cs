...
public partial class Admin_StaticPages : System.Web.UI.Page
{
    ...
    {
        Page.Title = string.Format("{0} - {1}", SettingsMain.ShopName, Resource.Admin_StaticPage_lblSubMain);
    ...
    protected void lbSetActive_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    StaticPageService.SetStaticPageActivity(Convert.ToInt32(id), true);
                }
            }
            else
            {
    ...
    protected void lbChangeParent_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    StaticPageService.ChangeParentPage(Convert.ToInt32(id), Convert.ToInt32(ddlParentPages.SelectedValue));
                }
            }
            else
            {
    ...
    protected void grid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            StaticPageService.DeleteStaticPage(Convert.ToInt32(e.CommandArgument));
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        try
        {
            if (grid.UpdatedRow != null)
            {
                ...
                {
                    var page = StaticPageService.GetStaticPage(Convert.ToInt32(grid.UpdatedRow["ID"]));
                    page.PageName = grid.UpdatedRow["PageName"];
                    page.Enabled = Convert.ToBoolean(grid.UpdatedRow["Enabled"]);
