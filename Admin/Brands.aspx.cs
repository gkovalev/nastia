...
public partial class Admin_Brands : System.Web.UI.Page
{
    ...
    {
        Page.Title = string.Format("{0} - {1}", SettingsMain.ShopName, Resources.Resource.Admin_Brands_Header);
        ...
        if (!IsPostBack)
        {
            ...
            _paging.AddParam(new SqlParam { ParamValue = PhotoType.Brand.ToString(), ParamName = "@Type" });
    ...
    protected void lbDeleteSelected_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                ...
                {
                    BrandService.DeleteBrand(Convert.ToInt32(id));
                }
            }
            else
            {
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (grid.UpdatedRow != null)
        {
            ...
            Brand brand = BrandService.GetBrandById(Convert.ToInt32(grid.UpdatedRow["ID"]));
            brand.Name = grid.UpdatedRow["BrandName"];
            brand.Enabled = Convert.ToBoolean(grid.UpdatedRow["Enabled"]);
