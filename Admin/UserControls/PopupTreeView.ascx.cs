...
public partial class Admin_UserControls_PopupTreeView : UserControl
{
    ...
    protected void LoadRoot()
    {
        ...
        switch (Type)
        {
            case eTreeType.MainMenu:
                ...
                foreach (var parentMenuItem in MenuService.GetParentMenuItems(Convert.ToInt32(Page.Request["menuid"]), MenuService.EMenuType.Top))
                {
                    var tn =
            ...
            case eTreeType.Category:
                ...
                var parentCategories = CategoryService.GetParentCategories(Convert.ToInt32(Page.Request["categoryid"]));
    ...
    private void LoadChildStaticPages(TreeNode node)
    {
        foreach (var childNode in
            from page in StaticPageService.GetChildStaticPages(Convert.ToInt32(node.Value), false)
    ...
    {
        foreach (var c in CategoryService.GetChildCategoriesByCategoryId(Convert.ToInt32(node.Value), false).Where(c => c.CategoryId != ExceptId && !AnotherExceptIds.Contains(c.CategoryId)))
        {
    ...
    {
        foreach (var c in CategoryService.GetChildCategoriesAndProducts(Convert.ToInt32(node.Value)).Where(c => c.Type == CatalogItemType.Category || (c.Id != ExceptId && !AnotherExceptIds.Contains(c.Id))).OrderBy(item => item.Type))
        {
    ...
    {
        foreach (var c in MenuService.GetChildMenuItemsByParentId(Convert.ToInt32(node.Value), MenuService.EMenuType.Top))
        {
            var newNode = new TreeNode
            {
                Text = c.MenuItemName,
