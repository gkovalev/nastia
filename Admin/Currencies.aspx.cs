...
public partial class Currencies : Page
{
    ...
    {
        Page.Title = string.Format("{0} - {1}", SettingsMain.ShopName, Resource.Admin_Currencies_Header);
    ...
    protected void lbDeleteSelected_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            ...
            if (!_inverseSelection)
            {
                ...
                {
                    if (currency.CurrencyID != Convert.ToInt32(id))
                    {
                        CurrencyService.DeleteCurrency(Convert.ToInt32(id));
                    }
                }
            }
            else
    ...
    protected void grid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            var currency = CurrencyService.Currency(SettingsCatalog.DefaultCurrencyIso3);
            if (currency.CurrencyID != Convert.ToInt32(e.CommandArgument))
            {
                CurrencyService.DeleteCurrency(Convert.ToInt32(e.CommandArgument));
                CurrencyService.RefreshCurrency();
            }
        }
        if (e.CommandName == "AddCurrency")
        {
            ...
            decimal temp;
            decimal.TryParse(((TextBox)footer.FindControl("txtNewCurrencyValue")).Text, out temp);
            if (
