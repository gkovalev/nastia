...
public partial class Admin_301Redirects : System.Web.UI.Page
{
    SqlPaging _paging;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Page.Title = string.Format("{0} - {1}", AdvantShop.Configuration.SettingsMain.ShopName, Resources.Resource.Admin_301Redirects_Header);
    ...
    protected void lbDeleteSelected_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            ...
            if (!_inverseSelection)
            {
                ...
                {
                    RedirectSeoService.DeleteRedirectSeo(Convert.ToInt32(id));
                }
            }
            else
            {
    ...
    protected void grid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        {
            RedirectSeoService.DeleteRedirectSeo(Convert.ToInt32(e.CommandArgument));
        }
        if (e.CommandName == "AddRedirectSeo")
        {
            GridViewRow footer = grid.FooterRow;
            var txtNexRedirectFrom = ((TextBox)footer.FindControl("txtNexRedirectFrom")).Text.Trim().Trim('/').ToLower();
            var txtNewRedirectTo = ((TextBox)footer.FindControl("txtNewRedirectTo")).Text.Trim().Trim('/').ToLower();
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (grid.UpdatedRow != null)
        {
            var redirectSeo = new RedirectSeo
            {
                ID = Convert.ToInt32(grid.UpdatedRow["ID"]),
                RedirectFrom = grid.UpdatedRow["RedirectFrom"].Trim().Trim('/').ToLower(),
                RedirectTo = grid.UpdatedRow["RedirectTo"].Trim().Trim('/').ToLower(),
