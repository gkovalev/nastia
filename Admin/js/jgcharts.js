...
jGCharts.Api = function () {
    ...
    function _color() {
        ...
        for (var x = 0; x < i; x++) {
            if (_colors[x]) {
                if (_colors[x].constructor.toString().indexOf("Array") != -1) {
                    _t = "";
                    for (y = 0; y < _datarow; y++) {
                        if (y < _palette.length)
                            _t += (_colors[x][y] || _palette[y]) + "|";
                        else
                            _t += _colors[x][y] + "|";
                    }
                    _t = _rlasttrim(_t, "|");
...
// Maps (t)
// cht=<line chart type>
jGCharts.Api.type = ["bhs", "bvs", "bhg", "bvg", "bvo", "bho", "lc", "ls", "p", "p3"]; //TODO: "r","v","s","t","gom","lxy"
