...
public partial class Admin_OrderSearch : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            _paging.AddFieldsRange(
                new Field("[Order].OrderID as ID") { IsDistinct = true, Sorting = SortDirection.Descending },
    ...
    {
        return CatalogService.GetStringPrice((decimal)sum, (decimal)rate, currency);
    }
    ...
    protected void lbSetPay_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    OrderService.PayOrder(Convert.ToInt32(id), true);
                }
            }
            else
            {
    ...
    protected void lbSetNotPay_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    OrderService.PayOrder(Convert.ToInt32(id), false);
                }
            }
            else
            {
