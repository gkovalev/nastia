...
public partial class Admin_StaticBlocks : System.Web.UI.Page
{
    ...
    {
        Page.Title = string.Format("{0} - {1}", SettingsMain.ShopName, Resource.Admin_PageParts_StaticBlocks);
        if (!IsPostBack)
        {
    ...
    protected void lbSetActive_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    StaticBlockService.SetStaticBlockActivity(Convert.ToInt32(id), true);
                }
            }
            else
            {
    ...
    protected void lbSetDeactive_Click(object sender, EventArgs e)
    {
        if ((_selectionFilter != null) && (_selectionFilter.Values != null))
        {
            if (!_inverseSelection)
            {
                foreach (var id in _selectionFilter.Values)
                {
                    StaticBlockService.SetStaticBlockActivity(Convert.ToInt32(id), false);
                }
            }
            else
            {
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        {
            var part = StaticBlockService.GetPagePart(Convert.ToInt32(grid.UpdatedRow["ID"]));

            part.InnerName = grid.UpdatedRow["InnerName"];
            part.Key = grid.UpdatedRow["[Key]"];
            part.Enabled = Convert.ToBoolean(grid.UpdatedRow["Enabled"]);
