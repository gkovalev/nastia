...
public partial class BrandView : AdvantShopPage
{
    ...
    private void LoadBrand()
    {
        ...
        else
        {
            ...
            foreach (var category in CategoryService.GetChildCategoriesByCategoryId(0, false).Where(cat => avalibleCategories.ContainsKey(cat.CategoryId)))
            {
                ...
                sb.AppendFormat( "<span class=\"title\"><a href='{0}'>{1} ({2})</a></span>", link, category.Name, avalibleCategories[category.CategoryId].InChildsCategoryCount);

                sb.AppendFormat("<div class=\"content list-brand-marker\">");

                foreach (var child in category.ChildCategories.Where(ch => avalibleCategories.ContainsKey(ch.CategoryId)))
                {
                    ...
                    sb.AppendFormat("<a href='{0}'>{1} ({2})</a>", sublink, child.Name, avalibleCategories[child.CategoryId].InChildsCategoryCount);
                }
                sb.AppendFormat("</div>");
                sb.AppendFormat("</div>");
            }
    ...
    private void LoadData()
    {
        ...
        sb.AppendFormat("<a href=\"manufacturers\" class=\"all-letter{0}\">{1}</a>", Request["letter"] == null ? " selected" : string.Empty, Resources.Resource.Client_Brands_All);
